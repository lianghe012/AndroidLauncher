...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	Log.w("super_hw", "DeleteDropTarget onDragStart()");
//    	Log.i("super_hw", "DeleteDropTarget onDragStart(), info:" + info.toString() + ", source:" + source.toString());
        ...
        if (!willAcceptDrop(info) || isAllAppsWidget(source, info)) {
        	if(info instanceof AppInfo){
        		Log.d("super_hw", "DeleteDropTarget onDragStart() isVisible = false; flags : " + ((AppInfo)info).flags);
        	} else if(info instanceof PendingAddWidgetInfo){
        		Log.d("super_hw", "DeleteDropTarget onDragStart() info instanceof PendingAddWidgetInfo");
        	}
            isVisible = false;
        }
        super_hw:
        if(LauncherAppState.getInstance().isPSLauncher){
        	if(info instanceof ShortcutInfo){
        		if(((ShortcutInfo) info).appInfo != null){
        			useUninstallLabel = ((ShortcutInfo) info).appInfo.flags == 1;
        		} else if(((ShortcutInfo) info).itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT){
        			// TODO 快捷键
        			isVisible = true;
        			useDeleteLabel = true;
    ...
    public void onDragEnd() {
        super.onDragEnd();
        Log.w("super_hw", "DeleteDropTarget onDrageEnd()");
        mActive = false;
    }

    public void onDragEnter(DragObject d) {
        super.onDragEnter(d);
        Log.w("super_hw", "DeleteDropTarget onDragEnter()");
        setHoverColor();
    }

    public void onDragExit(DragObject d) {
        super.onDragExit(d);
        Log.w("super_hw", "DeleteDropTarget onDragExit()");
        if (!d.dragComplete) {
            resetHoverColor();
        } else {
    ...
    private void animateToTrashAndCompleteDrop(final DragObject d) {
        ...
        Runnable onAnimationEndRunnable = new Runnable() {
            ...
            public void run() {
            		/** TODO 拖动完成 */
                Log.d("super_hw", "DeleteDropTarget animateToTrashAndCompleteDrop(), drag end - next to completeDrop(d)");
                completeDrop(d);
    ...
    private AppInfo ShortcutInfo2AppnInfo(ShortcutInfo sInfo){
     	return new AppInfo(sInfo);
     }
    ...
    private void completeDrop(DragObject d) {
    	Log.w("super_hw", "DeleteDropTarget completeDrop()");
        ItemInfo item = (ItemInfo) d.dragInfo;
        Log.i("super_hw", "DeleteDropTarget_completeDrop(), d.dragInfo : " + d.dragInfo + " ,dragView:" + d.dragView);
        if(item instanceof ShortcutInfo){
        	switch(((ShortcutInfo)item).itemType){
        	case LauncherSettings.Favorites.ITEM_TYPE_APPLICATION:
        		item = ShortcutInfo2AppnInfo((ShortcutInfo)item);
        		if(((AppInfo)item).getFlag() == 1) {
        			/** TODO 这个位置判断拖动结束后的事件 */
        			mLauncher.startApplicationUninstallActivity(((AppInfo)item).componentName, ((AppInfo)item).flags, ((AppInfo)item).user);
        			return;
        		} else {
        		}
        		break;
        	case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
        		// TODO 快捷键图标中的appInfo属性为空
        		Log.e("super_hw", "DeleteDropTarget completeDrop(),ITEM_TYPE_SHORTCUT case here");
        		break;
        	}
 	        }
        ...
        if (isAllAppsApplication(d.dragSource, item)) {
            ...
            if(appInfo.getFlag() == 1){
            	mLauncher.startApplicationUninstallActivity(appInfo.componentName, appInfo.flags,
            			appInfo.user);
            	return;
            } else if(appInfo.getFlag() == 0){
            	try {
            		if(LauncherAppState.getInstance().mLauncher != null)
            			LauncherAppState.getInstance().mLauncher.resumeEvent();
				} catch (Exception e) {
					e.printStackTrace();
				}
            }
        	...
        	try {
        		...
        		if(d.dragView == null && item.itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT){
        			// TODO 快捷键被删除
        			if(((ShortcutInfo)item).intent != null && ((ShortcutInfo)item).intent.getComponent() != null)
	        			if(LauncherAppState.getInstance().mLauncher != null)
	        				LauncherAppState.getInstance().mLauncher.removeDragView(((ShortcutInfo)item).intent.getComponent().getClassName());
        		}
			} catch (Exception e) {
				e.printStackTrace();
			}
...
//            LauncherModel.deleteFolderContentsFromDatabase(mLauncher, folderInfo);
            ...
            if(workspace != null){
            	for(int cIndex = 0;cIndex < workspace.getPageCount();cIndex++){
            		CellLayout cLayout = (CellLayout)workspace.getPageAt(cIndex);
            		try {
            			ViewGroup vg = (ViewGroup)cLayout.getChildAt(1);
            			for(int ccIndex = 0;ccIndex < vg.getChildCount();ccIndex++){
            				if(!(vg.getChildAt(ccIndex) instanceof LauncherAppWidgetHostView))
            					continue;
            				LauncherAppWidgetHostView widgetView = (LauncherAppWidgetHostView)vg.getChildAt(ccIndex);
            				if(widgetView.getTag() == null)
            					continue;
            				LauncherAppWidgetInfo cWidgetInfo = (LauncherAppWidgetInfo)widgetView.getTag();
            				if(cWidgetInfo.appWidgetId != widgetInfo.appWidgetId)
            					continue;
            				vg.removeView(widgetView);
            				break;
            			}
					} catch (Exception e) {
						e.printStackTrace();
    ...
    public void onFlingToDelete(final DragObject d, int x, int y, PointF vel) {
    	Log.d("super_hw", "Deletedroptarget onFlingToDelete line 513");
        ...
        Runnable onAnimationEndRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (!isAllApps) {
                    ...
                    Log.d("super_hw", "Deletedroptarget completeDrop, line 572");
                    completeDrop(d);
                }
                Log.d("super_hw", "Deletedroptarget completeDrop, line 575");

...
//            bubbleChild.setTextVisibility(!mIsHotseat);
        ...
        if(mIsHotseat){
    		Log.e("addViewToCellLayout","lp.cellX>>"+lp.cellX);
    		Log.e("addViewToCellLayout","lp.cellX <= mCountX - 1>"+(lp.cellX <= mCountX - 1));
    		Log.e("addViewToCellLayout","lp.cellY >= 0>>"+(lp.cellY >= 0));
    		Log.e("addViewToCellLayout","lp.cellY <= mCountY - 1>>"+(lp.cellY <= mCountY - 1));
    	}
        
        if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0 && lp.cellY <= mCountY - 1) {
        	if(mIsHotseat){
        		Log.e("addViewToCellLayout","mIsHotseat");
        	}
    ...
    public boolean addViewToCellLayoutFolder(View child, int index, int childId, LayoutParams params,
            boolean markCells) {
        final LayoutParams lp = params;
        // Hotseat icons - remove text
        if (child instanceof BubbleTextView) {
...
//            bubbleChild.setTextVisibility(!mIsHotseat);
        ...
        // per workspace screen
        if(misFolded){
    		Log.e("misFolded","lp.cellX>>"+lp.cellX);
    		Log.e("misFolded","lp.cellX <= mCountX - 1>"+(lp.cellX <= mCountX - 1));
    		Log.e("misFolded","lp.cellY >= 0>>"+(lp.cellY >= 0));
    		Log.e("misFolded","lp.cellY <= mCountY - 1>>"+(lp.cellY <= mCountY - 1));
    	}
...
//    	Log.v("super_hw", "CellLayout OnInterceptTouchEvent");
    ...
    */
     ...
     * @param result An array of length 2 in which to store the result (may be null).
     */
    public static int[] rectToCellForWidget(int width, int height, int[] result) {
        LauncherAppState app = LauncherAppState.getInstance();
        DeviceProfile grid = app.getDynamicGrid().getDeviceProfile();
        int columns = (int) grid.numColumns;
        Rect padding = grid.getWorkspacePadding(grid.isLandscape ?
                CellLayout.LANDSCAPE : CellLayout.PORTRAIT);
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
    	private String TAG = "LayoutParams";
        ...
        public void setMisHost(boolean b){
        	this.misHost = b;
        }
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap,
            ...
            if (isLockedToGrid) {
                ...
                if(misHost && DBUG){
                	Log.e(TAG,"cellWidth>>"+ cellWidth);
                	Log.e(TAG,"cellHeight>>"+cellHeight);
                	Log.e(TAG,"myCellX>>"+ myCellX);
                	Log.e(TAG,"myCellY>>"+myCellY);
                	Log.e(TAG,"x>>"+ x );
                	Log.e(TAG,"y>>"+y);
                }
            }
	...
	public boolean isMisFolded() {
		return misFolded;
	}

	public void setMisFolded(boolean misFolded) {
	 	Log.e("hahaF","setMisFolded>>"+misFolded);
		this.misFolded = misFolded;
	    mShortcutsAndWidgets.setmIsFoldedLayout(misFolded);
	}
}

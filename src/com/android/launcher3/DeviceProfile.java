...
public class DeviceProfile {
    ...
    int cellWidthPxNew;//workspace的中子view实际宽度
    int cellHeightPxNew;//workspace的中子view实际高度
...
//        }
    ...
    DeviceProfile(Context context,
                  ...
                  ArrayList<DeviceProfile> profilesPort,
        ...
        boolean isLandscapeTmp = (configuration.orientation == Configuration.ORIENTATION_LANDSCAPE);
    	Log.e("屏幕","isLandscapeTmp>>>"+ isLandscapeTmp);
//    	Log.e("屏幕","profilesPort.size>>>"+ profilesPort.size());
        if(isLandscapeTmp){
        	Log.e("屏幕","横屏");
            ...
            for (DeviceProfile p : profiles) {
        	  ...
        	  // Find the closes profile given the width/height
        	Log.e("屏幕","竖屏");
            for (DeviceProfile p : profilesPort) {
                points.add(new DeviceProfileQuery(p, 0f));
            }
        }
//    	Log.e("屏幕","points.size>>>"+ points.size());
//    	for(DeviceProfileQuery deviceProfile : points){
//    		Log.e("屏幕",""+deviceProfile.profile.);
//    	}
        /**
         * 根据分辨率找到对应的DeviceProfile对象
         */
        DeviceProfile closestProfile = findClosestDeviceProfile(minWidth, minHeight, points);
        Log.e("屏幕","closestProfile>>>"+closestProfile);
        ...
        /**
         *之前全部应用入口点在numHotseatIcons / 2
         *现在改成－1.
         */
//        hotseatAllAppsRank = (int) (numHotseatIcons / 2);
        hotseatAllAppsRank = -1;
...
//        }
        ...
        Log.e("初始化相关变量","numRows>>>"+numRows);
        Log.e("初始化相关变量","numColumns>>>"+numColumns);
        Log.e("初始化相关变量","numHotseatIcons>>>"+numHotseatIcons);
        Log.e("初始化相关变量","iconSize>>>"+iconSize);
        Log.e("初始化相关变量","iconTextSize>>>"+iconTextSize);
        Log.e("初始化相关变量","iconDrawablePaddingOriginalPx>>>"+iconDrawablePaddingOriginalPx);
        Log.e("初始化相关变量","hotseatIconSize>>>"+hotseatIconSize);
    ...
    private void updateAvailableDimensions(Context context) {
        ...
        Log.e("初始化相关变量","drawablePadding>>>"+drawablePadding);
        Log.e("初始化相关变量","usedHeight>>>"+usedHeight);
        
        Rect workspacePadding = getWorkspacePadding();
        int maxHeight = (availableHeightPx - workspacePadding.top - workspacePadding.bottom);
        Log.e("初始化相关变量","maxHeight>>>"+maxHeight);
...
//            }
            ...
            int height = (orientation != CellLayout.LANDSCAPE)
                    ...
                    /**
                     * 以前搜索框在workspace上。
                     * 所以设置workspace距离顶部为searchBarBounds.bottom
                     */
//            int paddingTop = searchBarBounds.bottom;
            ...
            padding.set(availableWidth / 7, 1,
                    availableWidth / 7, paddingBottom-10);
            Log.e("计算padding","paddingBottom>>"+paddingBottom);
            Log.e("计算padding","availableWidth>>"+availableWidth);
            Log.e("计算padding","widt>>>"+width);
            Log.e("计算padding","height>>>"+height);
        } else {
        	   float gapScale = 1f + (dragViewScale - 1f) / 2f;
               int width = (orientation == CellLayout.LANDSCAPE)
                       ? Math.max(widthPx, heightPx)
...
//               int paddingTop = searchBarBounds.bottom;
               ...
               int availableHeight = Math.max(0, height - paddingTop - paddingBottom
        		...
        		dm = LauncherAppState.getInstance().mLauncher.getResources().getDisplayMetrics();  
        		int densityDPI = dm.densityDpi;// 屏幕密度（每寸像素：120/160/240/320） 
               if(densityDPI < 240){
                   padding.set(availableWidth / 2, paddingTop+30,
                           availableWidth / 2, paddingBottom + availableHeight / 4+30);   
               }else{
...
//            }
    ...
    public void layout(Launcher launcher) {
        ...
        cellHeightPxNew = (int) ((availableHeightPx - padding.top - padding.bottom) / numRows);
        Log.e("test","cellHeightPxNew>>>"+cellHeightPxNew);
        ...
        if (hasVerticalBarLayout) {
            ...
            lp.height = hotseatBarHeightPx-6;
            ...
            if(!isLandscape){
            	 lp.bottomMargin = 20;
            		if(densityDPI < 240){
            			hotseatBarHeightPx += 60;
            		}
            }
            lp.height = hotseatBarHeightPx;
            /**
             * hotseat由距离底部2 * edgeMarginPx改成零
             */
//            hotseat.setPadding(edgeMarginPx + padding.left, 0,
//                    edgeMarginPx + padding.right,
//                    2 * edgeMarginPx);
            hotseat.setPadding(padding.left, 0,
                    padding.right,
                    0);
        } else {
        ...
        if (pageIndicator != null) {
            if (hasVerticalBarLayout) {
                ...
                if(!isLandscape){
                	   ...
                	   lp.bottomMargin = hotseatBarHeightPx-10;
                }

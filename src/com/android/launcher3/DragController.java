...
public class DragController {
    ...
    public void startDrag(View v, Bitmap bmp, DragSource source, Object dragInfo, int dragAction,
    	...
    	Log.i("super_hw", "DragController startDrag(), line 175");
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
    	...
    	Log.i("super_hw", "DragController startDrag(), line 210, source : " + source.toString());
        ...
        Log.i("super_hw", "DragController startDrag(), line 234 for new DragObject()");
    ...
    public void onAppsRemoved(final ArrayList<String> packageNames, HashSet<ComponentName> cns) {
    	Log.i("super_hw", "DragController onAppsRemoved()");
    ...
    private void endDrag() {
    	Log.i("super_hw", "DragController endDrag()");
        if (mDragging) {
            mDragging = false;
            clearScrollRunnable();
    ...
    private void dropOnFlingToDeleteTarget(float x, float y, PointF vel) {
    	Log.i("super_hw", "DragController dropOnFlingToDeleteTarget()");
        final int[] coordinates = mCoordinatesTemp;
    ...
    private void drop(float x, float y) {
    	Log.w("super_hw", "DragController drop()");
        final int[] coordinates = mCoordinatesTemp;
//        final by super_hw:从文件夹拖拽,甩至顶部触发删除操作
        DropTarget dropTarget = findDropTarget((int) x, (int) y, coordinates);

        mDragObject.x = coordinates[0];
        mDragObject.y = coordinates[1];
        boolean accepted = false;
        boolean uninstallEvent = false;
        if(mDragObject.dragSource instanceof Folder && dropTarget instanceof DeleteDropTarget){
        	Log.e("super_hw", "DragController_drop(), source is folder, dropTarget is DeleteDropTarget");
        	dropTarget = (Folder)mDragObject.dragSource;
        } else if (dropTarget instanceof DeleteDropTarget) {
        	if (mDragObject.dragView != null && mDragObject.dragInfo != null) {
        		if(mDragObject.dragInfo instanceof ShortcutInfo){
        			try {
						ShortcutInfo dInfo = (ShortcutInfo) mDragObject.dragInfo;
						if (dInfo.appInfo != null
								&& dInfo.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION
								&& dInfo.appInfo.getFlag() == 1) {
							uninstallEvent = true;
							if(mDragObject.dragSource instanceof Workspace){
								dropTarget = (Workspace)mDragObject.dragSource;
							} else if(mDragObject.dragSource instanceof Folder){
								dropTarget = (Folder)mDragObject.dragSource;
							}
							final int[] superCoordinates = mCoordinatesTemp;
							findDropTarget(mMotionDownX, mMotionDownY, coordinates);
							mDragObject.x = superCoordinates[0];
							mDragObject.y = superCoordinates[1];
							dropTarget.onDragEnter(mDragObject);
							if(dInfo.container == -101){
							   dropTarget.onDragOver(mDragObject);
							}
						}
					} catch (Exception e) {
        ...
        } else if(dropTarget instanceof InfoDropTarget) {
        	InfoDropTarget tempDropTarget = (InfoDropTarget)dropTarget;
        	if (mDragObject.dragView != null && mDragObject.dragInfo != null) {
        		if(mDragObject.dragInfo instanceof ShortcutInfo){
        			try {
						ShortcutInfo dInfo = (ShortcutInfo) mDragObject.dragInfo;
...
//								&& dInfo.appInfo.getFlag() == 1
							...
							tempDropTarget.startApplicationDetailActivity(mDragObject);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
        ...
        if(uninstallEvent) {
        	uninstallEvent = false;
        	uninstallEvent(mDragObject);
        }
    }

    private void uninstallEvent(DropTarget.DragObject d){
    	try {
    		if(d != null)
				if(d.dragView != null && d.dragInfo != null && d.dragInfo instanceof ShortcutInfo){
					if(((ShortcutInfo) d.dragInfo).appInfo != null){
						mLauncher.startApplicationUninstallActivity(((ShortcutInfo) d.dragInfo).appInfo.componentName,
								((ShortcutInfo) d.dragInfo).appInfo.flags, ((ShortcutInfo) d.dragInfo).appInfo.user);
					}
				}
		} catch (Exception e) {
			e.printStackTrace();
